...
helcount = 0 #keep track of visits to hello page
...
def handle_http_get_whoami(conn):
    log("Handling http get whoami request")
    print(conn.leftover_data)
    msg = "<html><head><title>Server Info</title><head>"
    msg += "<body>"
    msg += f"<p> Client IPv4 address: {conn.client_addr}"
    msg += f"<p> Using port no. %s " % (s.getnameinfo(conn.client_addr))
    msg += "</body></html>"
    return Response("200 OK", "text/html", msg)

def handle_http_get_hello_parameters(req):
    username = "World"
    favorite = "teal"
    if "?" in req.path:
        req.path, params = req.path.split("?", 1)
...
def handle_http_get_hello(req, conn):
    ...
    helcount += 1
    # handle parameters
    username, favorite = handle_http_get_hello_parameters(req)
    trucolors = [ "coral", "crimson", "navy", "purple", "tomato", "teal", "wheat" ]
    if favorite not in trucolors:
        favorite = random.choice(trucolors)
        switch = True
    background = f"background-color:{favorite};"
    msg = f'<html><head><title>Welcome, {username}</title></head>'
    msg += f'<body style={background}><p>The current date is %s.\n' % (time.strftime("%a, %d %b %Y %H:%M:%S %Z"))
    msg += '<p>You have visited this page %s time(s) before\n' % (helcount)
    if switch == True:
        msg += "<p> Your favorite color is not a real color: Only these are: coral, crimson, navy, purple, tomato, teal, wheat"
        msg += "<p>It's okay! I picked a random one for you"
    else:
        msg += f'<p>Your favorite color is... {favorite}!\n'
    msg += '<p>Click <a href=></a>here to see something crazy!!\n'
    msg += '<p>You can also go to these exciting pages:\n'
    msg += f"<p>  http://{server_host}:{server_port}/hello        - this page\n"
    msg += f"<p>  http://{server_host}:{server_port}/status       - status and statistics\n"
    msg += f"<p>  http://{server_host}:{server_port}/quote        - a random quote\n"
    msg += f"<p>  http://{server_host}:{server_port}/welcome.html - a welcome page\n"
    msg += f"<p>  http://{server_host}:{server_port}/index.html   - some fancy web page\n"
    msg += "\n"
    msg += f'<form action="http://{server_host}:{server_port}/hello?username=name&favorite=fave"> <p><label for="name">Enter your name:\n </label>'
    msg += '<input type="text" id="name" name="username"><br><br> '
    msg += '<p><label for="name">Favorite color:\n </label>'
    msg += '<input type="text" id="fave" name="favorite"><br><br>'
    msg += '<input type="submit" value="Submit"></form>'
    msg += "<p>Have the best day ever!\n </body></html>" 
...
def handle_http_get_dir(url_path):
    ...
    directory = os.listdir(server_root + url_path)
    msg = "<html><head><title>404 NOT FOUND</title></head>"
    msg += "<body>"
    msg += "<p> index.html does not exist. Here is a list of files in that directory\n"
    msg += "<ul>"
    for x in range(len(directory)):
        msg += f'<li>{directory[x]}</li>'
    msg += "</ul>"
    msg += "</body></html>"
    return Response("200 OK", "text/html", msg)
...
def handle_http_get_file(url_path):
    ...
    if url_path == "/" or url_path == "/index.html":
        file_path = server_root + "/index.html" if url_path == "/" else server_root + url_path
        if not os.path.isfile(file_path):
            return handle_http_get_dir(url_path)
    else:
